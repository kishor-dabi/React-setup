...
class RegistrationForm extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.successMessage) {
      this.setState({ error: nextProps.successMessage, isOpenDialog: true })
      setTimeout(() => {
        this.setState({ error: "", isOpenDialog: true })
        clearError();
      }, 5000);
    }
    if (nextProps.error) {
      this.setState({ error: nextProps.error, isOpenDialog: true })
      setTimeout(() => {
        this.setState({ error: "", isOpenDialog: true })
        clearError();
      }, 5000);
